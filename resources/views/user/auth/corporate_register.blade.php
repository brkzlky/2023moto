			...
			<div class="row">
				<div class="col-lg-4 pe-lg-0 mb-4 mb-lg-0">
					<div class="c-auth__form">
						...
						<form action="{{ route('member.doCorporateRegister') }}" method="POST">
                            @csrf
                            <div class="c-auth__form-row">
                                <input type="text" class="form-control c-input text-transform-unset" name="fullname" placeholder="{{ __('module/label.corporate_title') }}" value="{{ old('fullname') }}" required>
                            </div>

                            <div class="c-auth__form-row">
                                <input type="text" class="form-control c-input text-transform-unset" name="phone" placeholder="{{ __('module/label.phone') }}" value="{{ old('phone') }}" required>
                            </div>
                            ...
                            <div class="c-auth__form-row">
                                <select name="country" class="form-control c-input countryselect" value="{{ old('country') }}" placeholder="{{ __('module/label.country') }}" required>
                                    <option value="">{{ __('module/label.country') }}</option>
                                    @foreach($countries as $c)
                                    <option value="{{ $c->country_guid }}">{{ $c->name }}</option>
                                    @endforeach
                                </select>
                            </div>
