					...
					},
					{
						...
						value: function (t) {
							...
							var e,
								...
								c =
									((e = this.$content),
									Math.max(parseFloat(e.naturalWidth || 0), parseFloat((e.width && e.width.baseVal && e.width.baseVal.value) || 0), parseFloat(e.offsetWidth || 0), parseFloat(e.scrollWidth || 0))),
								h = (function (t) {
									return Math.max(
							...
							if (
								((m -= parseFloat(b.paddingLeft) + parseFloat(b.paddingRight)), (y -= parseFloat(b.paddingTop) + parseFloat(b.paddingBottom)), (this.viewport.width = m), (this.viewport.height = y), r)
							) {
						...
						value: function () {
							if ('pointerdown' === this.state) {
								var t,
									...
									s = o.boundY;
								if (
									(a && ((t = this.dragPosition.x < a.from), (e = this.dragPosition.x > a.to)), s && ((i = this.dragPosition.y < s.from), (n = this.dragPosition.y > s.to)), (t || e) && (!t || !e))
								) {
									var r = t ? a.from : a.to,
			...
			return (
				s(t, [
					...
					},
					{
						...
						value: function () {
							this.$container ||
								((this.$container = document.createElement('div')), this.$container.classList.add(this.option('classNames.main')), this.carousel.$container.appendChild(this.$container)),
						...
						value: function (t) {
							if (!t) {
								var e,
									i = this.slides[0];
								if (
									(((t = document.createElement('div')).dataset.isTestEl = 1), (t.style.visibility = 'hidden'), t.classList.add(this.option('prefix') + this.option('classNames.slide')), i.customClass)
								)
				...
				s(t, [
					...
					},
					{
						...
						value: function (t) {
							...
							if (!e || 'string' == typeof e) {
								if (
									(n = e.match(/(?:youtube\.com|youtu\.be|youtube\-nocookie\.com)\/(?:watch\?(?:.*&)?v=|v\/|u\/|embed\/?)?(videoseries\?list=(?:.*)|[\w-]{11}|\?listType=(?:.*)&list=(?:.*))(?:.*)/i))
								) {
					...
					},
					{
						...
						value: function (t, e, i) {
							...
							'inline' === i.type &&
								n &&
								(n.classList.remove('fancybox__content'), 'none' !== n.style.display && (n.style.display = 'none'), i.$closeButton && (i.$closeButton.remove(), (i.$closeButton = null)));
					...
					},
					{
						...
						value: function () {
							if (
								...
								!this.fancybox.Carousel.prevPage && (this.fancybox.option('slideshow.autoStart') && this.Slideshow.activate(), this.fancybox.option('fullscreen.autoStart') && !Z.element()))
							)
								try {
