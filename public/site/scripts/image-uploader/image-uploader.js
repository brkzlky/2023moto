	...
	$.fn.imageUploader = function (options) {
		...
		let validateMaxFiles = function (index, file) {
			if (index + dataTransfer.items.length + plugin.settings.preloaded.length >= plugin.settings.maxFiles) {
				...
				let listingUploader = $('.listing-img-uploader');

				if (listingUploader.length > 0) {
					let maxFilesErrorBox =
						'<div class="c-input__error-block"><div class="c-input__error-icon"><svg class="c-icon__svg c-icon--sm"><use xlink:href="../assets/images/sprite.svg#warn-circle"></use></svg></div><p>You can maximum upload up to 10 photos.</p><div class="c-input__error-closer"><svg class="c-icon__svg c-icon--sm"><use xlink:href="../assets/images/sprite.svg#close"></use></svg></div></div>';
					document.querySelector('.listing-img-uploader').insertAdjacentHTML('afterend', maxFilesErrorBox);

					if (maxFilesErrorBox.length > 0) {
						$('.c-input__error-block').not('.c-input__error-block:first').remove();
					}
				}

				$('.c-input__error-closer').click(function () {
					$(this).parent().remove();
				});
